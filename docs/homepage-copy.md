...
ignore it and show a notice. BrowserViewport capture commands remain Page.getLayoutMetrics
and Page.captureScreenshot; bounded explicit key requests additionally use fixed
Input.dispatchKeyEvent keyDown/keyUp. NativeWindow does not attach Chrome debugger for capture.
...
## What it doesn't do
No DOM scraping, Runtime.evaluate, OCR, page-content analysis, arbitrary automated
click/input, text macros, ChatGPT output/completion extraction or network response inspection.
One structured allowlisted key/chord may be dispatched to the exact owned active tab;
completion does not establish page or browser-UI handling. Thumbnails are
...
- One bounded allowlisted key/chord request for the exact owned active tab; no macro or
  arbitrary remote-control route.
