...
export class MonitorManager {
  ...
  async recover() {
    ...
    const socket = new this.Socket(record.origin.replace('http:', 'ws:') + '/lazy-chrome-extension/monitor');
  ...
  remove(id) {
    ...
    if (!client) return;
    client.removed = true; client.socket.close(); void this.pump(client);
  }
  async detach(client) {
    ...
    try {
      while (!client.removed && client.control?.enabled && client.blocked !== client.control.generation) {
        ...
        if (client.socket.bufferedAmount < 1000000) client.send({ type: 'frame', generation: control.generation,
          windowId: client.record.windowId, tabId, data: frame.data, captureMilliseconds: performance.now() - started });
        await delay(Math.max(0, 1000 / control.options.framesPerSecond - (performance.now() - started)));
      }
    } catch (error) {
      client.blocked = client.control?.generation;
      client.send({ type: 'status', generation: client.control?.generation ?? 0, capturing: false, error: String(error.message).slice(0, 240) });
    } finally {
      await this.detach(client);
      client.send({ type: 'status', generation: client.control?.generation ?? 0, capturing: false });
