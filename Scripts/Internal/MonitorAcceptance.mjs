...
export async function testMonitor({ caller, cdp, until, delay, evidence, pageA, pageB }) {
  ...
  await start(a);
  // Keep one live capture during real BridgeRuntime shutdown, verifying detach before browser cleanup.
  ...
  console.log('PASS: monitor production monitor, changing offscreen frames, per-session isolation, navigation, restore, worker restart, start/stop, close and shutdown.');
}
