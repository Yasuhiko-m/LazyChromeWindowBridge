...
export async function testMultiMonitor({ caller, cdp, until, delay, evidence, pageA, pageB }) {
  ...
  await sample('R005-five-session-intended-use');
  evidence('R005-initial-identities',{windows:initialGeometry,topology});
  ...
  await sample('R005-worker-recovery-four-eligible-only');
  ...
  await sample('R005-navigation-isolation');
  ...
  await sample('R005-three-continue-after-restore',parkedRows.slice(1),4);
  assert.equal((await monitor(b)).frames,stoppedB.frames);
  await caller('park',{id:b.appSessionId});
  await sample('R005-repark-same-session',parkedRows,4);
  evidence('R005-restore-error',{maximumPixels:error(restored.current,b.normal),restoreAndDetachMilliseconds,restored});
  ...
  evidence('R005-manual-physical-bounds',{result:'PASS',before:aBefore,after:await geometry(a),actualNegativeMonitor:!!left,unaffectedB:await geometry(b)});
  const matrix=[];
  for(const [strategy,maxWidth,maxHeight] of [['normal',320,180],['small',320,180],['normal',240,135],['small',240,135],['normal',240,135]]) {
    ...
    await delay(1000);
    matrix.push({strategy,maxWidth,maxHeight,fps:2,...await sample('R005-comparison-'+strategy+'-'+maxWidth)});
  }
  evidence('R005-performance-matrix',{matrix,cpuBasis:'percent of one core; caller includes test-only frame hash sampling; Base64 excludes JSON/WebSocket headers'});
  ...
  await sample('R005-owned-active-tab-change',parkedRows,4);
  ...
  await sample('R005-close-isolation',parkedRows.slice(0,3),4);
  ...
  evidence('R005-clean-shutdown',{result:'PASS',shutdown,finalGeometry,restoreErrors:parkedRows.slice(0,3).map(s=>({id:s.appSessionId,pixels:error(finalGeometry.find(g=>g.appSessionId===s.appSessionId).current,s.normal)}))});
  console.log('PASS: R005 five sessions, four PARKED fresh isolated captures, ACTIVE zero capture, manual native bounds, comparisons, lifecycle and exact shutdown restore.');
}
