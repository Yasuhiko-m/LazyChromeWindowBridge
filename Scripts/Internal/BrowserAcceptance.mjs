...
const profile = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), 'LazyChromeWindowBridge-CfT-')));
...
try {
  ...
  createInterface({ input: driver.stdout }).on('line', line => {
    ...
    if (!line.startsWith('LCWB ')) { console.log('Caller child emitted a non-protocol launch diagnostic.'); return; }
  ...
  if (process.argv.includes('--geometry')) {
    const { testGeometry } = await import('./GeometryAcceptance.mjs');
  ...
  if (cdp.ws?.readyState === WebSocket.OPEN) {
    ...
    for (const target of targets.filter(t => t.type === 'page' && t.url.includes('/lazy-chrome-window-bridge/bootstrap'))) {
