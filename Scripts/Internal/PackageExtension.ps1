...
if ($manifest.version -cne '0.2.0') { throw 'Unexpected CWS version.' }
$artifactRoot = Join-Path $packageRoot 'artifacts/cws'
New-Item -ItemType Directory -Path $artifactRoot -Force | Out-Null
$artifact = Join-Path $artifactRoot 'LazyChromeWindowBridge.Extension-0.2.0-cws.zip'
[IO.File]::WriteAllBytes($artifact, $firstArchive)
[pscustomobject]@{check='extension-package';result='PASS';artifact='artifacts/cws/LazyChromeWindowBridge.Extension-0.2.0-cws.zip';files=$packageNames;bytes=$firstArchive.Length;sha256=$firstHash;repeatIdentical=$true;manifestVersion=$manifest.version;releaseVersion='0.2.0'} |
